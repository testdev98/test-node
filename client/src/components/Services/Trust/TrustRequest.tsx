...
import { trustService } from '@/api/services';
import TrustResponse from './TrustResponse';
...
function TrustRequest() {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    ip: "",
    request_id: "",
    envType: getServiceEnv(user, "Trust"),
  });

  useEffect(() => {
    setFormData(prev => ({ ...prev, envType: getServiceEnv(user, "Trust") }));
  }, [user]);
  ...
  const validateForm = useCallback((): ValidationError[] => {
    ...
    if (formData.ip && !isValidIP(formData.ip)) {
      newErrors.push({ field: "ip", message: "Please enter a valid IP address." });
    }
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        const response = await trustService(formData);
        ...
        toast({
          title: "Success",
          description: "Trust service response received successfully.",
        });
          ...
          <Card className="bg-gradient-to-br from-card/80 to-card/60 backdrop-blur-xl border-border/50 shadow-xl">
            <CardHeader>
              <div className="flex justify-between items-start w-full">
                <CardTitle>Trust Request</CardTitle>
                <EnvironmentSwitch serviceName="Trust" />
              </div>
            </CardHeader>
            <CardContent>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">{record.request.ip}</p>
                    </div>
                    <div>
